...
@EventBusSubscriber
public class ElderGuardianChanges {
   @SubscribeEvent
         ...
         if (!event.loadedFromDisk()) {
            if (!guardian.level().isClientSide) {
               if (entity.getClass() == ElderGuardian.class) {
                  if (day >= 40) {
                     if (!guardian.getTags().contains("fromGuardian")) {
                        guardian.setCustomName(Component.literal("Super Elder Guardian").withStyle(ChatFormatting.GOLD));
                     }
