...
public class LingeringShockRecipe extends CustomRecipe {
   ...
   public LingeringShockRecipe(CraftingBookCategory category) {
      super(category);
   }
   ...
   public ItemStack assemble(CraftingInput inv, Provider provider) {
      ...
      result.set(
         DataComponents.CUSTOM_NAME, Component.translatable("item.permadeath_reincarnated.lingering_potion_shock").withStyle(style -> style.withItalic(false))
      );
      result.setCount(1);
      return result;
   }
   ...
   public RecipeSerializer<?> getSerializer() {
      return (RecipeSerializer<?>)PermadeathRecipeSerializers.LINGERING_SHOCK.get();
   }
