...
public class PermadeathEnhancedTCND extends Item {
   public PermadeathEnhancedTCND(Properties properties) {
      super(properties);
   }
   ...
   public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
      ...
      TCNDCooldownAttachment subCooldown = TCNDCooldownAttachmentHelper.get(player);
      if (!level.isClientSide) {
         if (subCooldown.getRemainingTicks() > 0) {
            ...
            player.displayClientMessage(
               Component.literal("No puedes usar este item mientras tengas el poder de la ")
                  ...
                  .append(Component.literal("Leyenda").withStyle(ChatFormatting.GOLD).append(Component.literal(" activo.").withStyle(ChatFormatting.YELLOW))),
               true
            );
         ...
         if (cooldown.getRemainingTicks() <= 0) {
            player.addEffect(new MobEffectInstance(MobEffects.HEALTH_BOOST, 2400, 1, true, true, true));
            player.addEffect(new MobEffectInstance(MobEffects.REGENERATION, 300, 2, true, true, true));
            player.addEffect(new MobEffectInstance(MobEffects.DAMAGE_RESISTANCE, 400, 1, true, true, true));
            player.addEffect(new MobEffectInstance(MobEffects.SATURATION, 20, 0, true, true, true));
            player.sendSystemMessage(
               Component.literal("Sientes que el poder de ")
                  .withStyle(ChatFormatting.LIGHT_PURPLE)
                  .append(
                     Component.literal("una leyenda oscura")
                        .withStyle(ChatFormatting.DARK_PURPLE)
                        .append(Component.literal(" te da vida y resistencia...").withStyle(ChatFormatting.LIGHT_PURPLE))
                  )
            );
            ...
            if (level instanceof ServerLevel serverLevel) {
               serverLevel.sendParticles(ParticleTypes.SCULK_SOUL, player.getX(), player.getY() + 1.0, player.getZ(), 40, 0.6, 0.8, 0.6, 0.05);
               serverLevel.sendParticles(ParticleTypes.SOUL_FIRE_FLAME, player.getX(), player.getY() + 1.0, player.getZ(), 40, 0.6, 0.8, 0.6, 0.05);
            ...
            if (!formatted.isEmpty()) {
               ...
               player.displayClientMessage(
                  Component.literal("No puedes usar este item por ")
                     .withStyle(ChatFormatting.LIGHT_PURPLE)
                     .append(Component.literal(formatted).withStyle(ChatFormatting.DARK_PURPLE)),
                  true
               );
            }
         }
